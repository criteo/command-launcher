...
func initApp() repository.PackageRepository {
	...
	dropinRepo, err := repository.CreateDropinRepository(viper.GetString(config.DROPIN_FOLDER_KEY))
	if err != nil {
		log.Fatal(err)
	}
...
func addBuiltinCommands() {
	...
	AddListCmd(rootCmd, rootCtxt.appCtx)
	AddInstallCmd(rootCmd, rootCtxt.appCtx)
	AddUninstallCmd(rootCmd, rootCtxt.appCtx)
	AddUpdateCmd(rootCmd, rootCtxt.appCtx)
}
